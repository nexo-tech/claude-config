...
let
  ...
  # Theme patching for Go skill templates (Pico CSS)
  picoTheme = if cfg.themeMode == "dark" then "dark" else "light";

  patchPicoTheme = path:
    pkgs.writeText (builtins.baseNameOf path)
    (builtins.replaceStrings [ ''data-theme="light"'' ]
      [ ''data-theme="${picoTheme}"'' ] (builtins.readFile path));
...
in {
  options.programs.claude-config = {
    ...
    themeMode = lib.mkOption {
      ...
      default = "dark";
      description = "Theme mode for templates generated by Claude skills";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Go skills plugin (patched to match themeMode)
    home.file.".claude/go-skills-plugin" = {
      source = goSkillsPluginPath;
      recursive = true;
    };

    home.file.".claude/go-skills-plugin/skills/go-htmx-dashboard/SKILL.md".source =
      patchPicoTheme "${goSkillsPluginPath}/skills/go-htmx-dashboard/SKILL.md";

    home.file.".claude/go-skills-plugin/skills/go-templ-components/SKILL.md".source =
      patchPicoTheme
      "${goSkillsPluginPath}/skills/go-templ-components/SKILL.md";

    home.file.".claude/go-skills-plugin/skills/go-pico-embed/SKILL.md".source =
      patchPicoTheme "${goSkillsPluginPath}/skills/go-pico-embed/SKILL.md";

    home.file.".claude/go-skills-plugin/skills/go-project-bootstrap/SKILL.md".source =
      patchPicoTheme
      "${goSkillsPluginPath}/skills/go-project-bootstrap/SKILL.md";
